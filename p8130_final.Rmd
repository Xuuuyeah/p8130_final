...
# 类别变量分布
data_clean |>
  dplyr::select(race, marital_status, t_stage, n_stage) |>
  mutate(across(everything(), as.character)) |>
  pivot_longer(cols = everything(), names_to = "Variable", values_to = "Category") |>
  ggplot(aes(x = Category, fill = Variable)) +
  geom_bar(position = "dodge") +
  labs(title = "Categorical Variable Distribution") +
  theme_minimal()
```
...
data_transformed <- data_clean %>%
  mutate(
    # 对右偏的变量取 log1p 变换
    regional_node_examined = log(regional_node_examined),
...
data_transformed %>%
  ...
  theme_minimal() +
  ggtitle("Transformed Variable Distributions")
